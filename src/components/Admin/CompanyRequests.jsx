...
const CompanyRequests = () => {
      ...
      .then(data => {
        setRequests(prevRequests =>
          prevRequests.map(request =>
            request.user.id === id ? { ...request, status: 'active' } : request
          )
        );
      })
      .catch(error => {
  ...
  const handleReject = (id) => {
    ...
    fetch(`http://localhost:8080/admin/companyRequests/${id}/reject`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
      ...
      .then(data => {
        setRequests(prevRequests =>
          prevRequests.map(request =>
            request.user.id === id ? { ...request, status: 'rejected' } : request
          )
        );
      })
      .catch(error => {
    ...
    <Card>
      <CardBody className='px-4 pt-6' style={{ height: '450px' }}>
        <div className="flex flex-col gap-5">
          ...
          <Table aria-label="Company Requests">
            <TableHeader>
                  ...
                  <ModalBody className='flex flex-row'>
                    <div className='w-1/2'>
                      {selectedRequest && (
                        <>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Company Name:<br /></span> {selectedRequest.companyName}
                          </p>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Industry:<br /></span> {selectedRequest.industryType}
                          </p>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Company Location:<br /></span> {selectedRequest.location}
                          </p>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Contact Number:<br /></span> {selectedRequest.contactNumber}
                          </p>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Email:<br /></span> {selectedRequest.email}
                          </p>
                          <p className='mb-3'>
                            <span className='text-purple2 font-medium text-sm'>Website:<br /></span> <a className='text-blue-600' href={selectedRequest.companyWebsite} target="_blank" rel="noopener noreferrer">Click Here</a>
                          </p>
                        </>
                      )}
                    </div>
                      ...
                      {selectedRequest && (
                        <>
                          ...
                          <p className='mb-3'>
                            ...
                            <img src={selectedRequest.bannerImg} alt="Company Banner" style={{ height: '250px', width: 'auto' }} />
                          </p>
                        </>
                      )}
                    </div>
